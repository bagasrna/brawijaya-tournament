<template>
    <div>
        ...
        <Content title="Pengumuman">
            <Button @click="createPengumuman" class="btn-create" icon="ios-add">Buat Pengumuman</Button>
...
export default {
    ...
    methods: {
        async getData() {
            var res = await this.callApi("get", "/app/announcement");
        ...
        async createPengumuman() {
            var res = await this.callApi("post", "/app/announcement", {
                title: `pengumuman-${this.data.length + 1}`,
            });
            ...
            this.$Modal.confirm({
                ...
                onOk: async () => {
                    var res = await this.callApi(
                        "delete",
                        `/app/announcement/${this.data[index].id}`
                    );
